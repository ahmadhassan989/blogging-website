    ...
    <section class="px-6 py-8">
        <main class="max-w-xl mx-auto mt-10 bg-gray-100 p-6 border border-gray-200">
            ...
            <form action="/register" method="post">
                @csrf
                <div class="mb-6">
                    <label for="name" class="block uppercase font-bold text-xs text-gray-700 mb-2">full Name</label>
                    <input type="text" class="border border-gray-400 w-full p-2" name="name" value="{{old('name')}}" id="">
                    @error('name')
                        <p class="text-xs text-red-500 py-1" >{{$message}}</p>
                    @enderror
                </div>
                <div class="mb-6">
                    <label for="username" class="block uppercase font-bold text-xs text-gray-700 mb-2">user name</label>
                    <input type="text" class="border border-gray-400 w-full p-2" name="username" value="{{old('username')}}"  id="">
                    @error('username')
                    <p class="text-xs text-red-500 py-1" >{{$message}}</p>
                @enderror
                </div>
                ...
                <div class="mb-6">
                    <button type="submit" class="bg-blue-400 p-2 rounded text-white px-4 hover:bg-blue-600">Submit</button>
                </div>
            </form>
        </main>
    </section>
